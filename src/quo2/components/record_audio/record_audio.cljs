...
(defn record-button [frame recording? ready-to-send? ready-to-delete? locked?]
  [:f>
   (fn []
     (let [scale-1 (reanimated/use-shared-value 1)
       ...
       (quo.react/effect! #(if @locked? (start-x-y-animation) (reset-x-y-animation)) [@locked?])
