(ns status-im.ui.screens.wallet-connect.permissions.views
...
(defn permission-details [requested-session]
  requested-session)

(views/defview sessions-panel [[dapp? dapp dapps-account] {:keys [dapp-name]}]
  (views/letsubs [bottom-anim-value  (anim/create-value styles/panel-height)
                  alpha-value        (anim/create-value 0)
                  current-session    (reagent/atom nil)
                  update?            (reagent/atom nil)]
    {:UNSAFE_componentWillUpdate (fn [_ [_ _ {:keys [requested-session]}]]
                                   (cond
                                     @update?
                                     ...
                                     (and @current-session requested-session)
                                     ...
                                     ;; `current-session` with a delay so that the information is still
                                     ;; available during the animation
                                     (do (reset! update? true)
                                         (js/setTimeout #(reset! current-session
                                                                 (permission-details requested-session))
                                                        600)
                                         (hide-panel-anim bottom-anim-value alpha-value))

                                     requested-session
                                     ;; the dapp is asking for a permission, we put it in current-session
                                     ;; and start the show-animation
                                     (do (reset! current-session
                                     ...
                                     ;; `current-session` with a delay so that the information is still
                                     ;; available during the animation
                                     (do (js/setTimeout #(reset! current-session nil) 500)
                                         (hide-panel-anim bottom-anim-value alpha-value))))}
    (when @current-session
      (let [{:keys [title description type icon]} @current-session]
